...
para la opción de carga EV lenta y lo resuelve- entrega finalmente el valor resultante de la función objetivo tras la optimización
"""
#librerias
import itertools
import pandas as pd
...
def OptimizadorDA(adopcion, demanda, Potencia, Tmin_config, SeleccionEVs, PerfilResidencial,Energia,desconexiones_perfiles,n_sw,infactibles):
    ...
    # # m.params.TimeLimit = 200    # 12100 # second
    m.params.MIPGap = 0.03
    # m.params.DegenMoves=1
    # m.params.method = 2
    # m.params.Crossover=0
    # m.params.Presolve = 0      # Default: -1 automatic, 0 off
    # m.Params.MIPGap = 1e-3 # Default: 1e-4 (tolerancia de valor objetivo)
    ...
    N = range(n)      # 
    T = 144           # Nº intervalos temporales (resolucion de 10 mins)
    eta = 1           # Eficiencia (es 1 dado que los datos de energía objetivo por vehículo corresponden a energía retirada de la red)
    pLoad = 3.6       # En esta version del modelo la potencia es fija e idéntica para cada vehículo
    ...
    for k in range(len(infactibles.columns)):
        ...
        for j in range(n_infact):
            ini = infactibles.iloc[j,k]
            fin = infactibles.iloc[j+1,k]
    ...
    # Restriccion para satisfacer demanda energetica

    m.addConstrs(eta * pLoad * quicksum(p[i,t] for t in T_factible[i]) == EVs_data[i]*6 for i in N)
    # m.addConstrs(eta * pLoad * quicksum(p[i,t] for t in range(T)) <= EVs_data[i]*1.1 for i in N)
    # m.addConstrs(eta * pLoad * quicksum(p[i,t] for t in range(T)) >= EVs_data[i]*0.9 for i in N)
    ...
    for k in range(NumCols): # numero de vehiculos
        for j in range(len(desconexiones)-1):  # filas del archivo desconexión
            if desconexiones.iloc[j,k] == -1.0: #se llenaron los nan con -1
                # Se acabaron los datos de desconexion
                break
            elif (j % 2) - 1  == 0: 
                # Restricción: la potencia debe ser 0 para el período de desconexión del vehículo
                m.addConstrs(p[k,t] == 0 for t in range(T) if t in range( int(desconexiones.iloc[j-1,k]),1+int(desconexiones.iloc[j,k])))
           
    # Restricciones del tipo tiempo minimo de carga
    # Esto es por si se define por ejemplo Tmin=3 (30 mins) pero el vehículo requiere solo 10 mins de carga      
    Tmin = [min(Tmin_config, int(EVs_data[i]/(3.6/6))) for i in N]
        
 
    # Condicion inicial
    for i in N:
        #Chequeamos infactibilidad de carga 
    ...
    for i in N:
        for t in T_factible[i][1:-1]:
                if t < T-Tmin[i]: # Esto es, evaluamos si no nos encontramos justo en los intervalos finales del día (condición de borde)
                    ...
                    # Condición de borde:
                    m.addConstr(p[i,t+1] <= p[i,t]) #De m odo que si p[i,t] = 1, en t+1 se puede detener o continuar la carga
    ...
    m.addConstrs(pLoad * quicksum(p[i,t] for i in N) + dataCarga[t] <= max_power for t in range(T))
    ...
    m.addConstrs(quicksum(w[i,t] for t in T_factible[i]) + x[i] <= 3 for i in N) #limite de procesos de carga
    # # m.addConstrs(quicksum(b[i,t] for t in range(T)) <= n_sw[i] for i in N) #limite de procesos de carga
    ...
    dataExcel = pd.DataFrame({'EV'+str(i):[3.6*p[i,t].X for t in range(T)] for i in N})    
